...
function isAuthorHref(href: string, slug: string) {
  const segments = getHrefSegments(href);
  return segments.length === 2 && segments[0] === "author" && segments[1] === slug;
}
...
export async function generateMetadata({ params, searchParams }: PageProps<"/author/[slug]">): Promise<Metadata> {
  const [{ slug }, query] = await Promise.all([params, searchParams]);
  const author = await getAuthorBySlug(slug);
  return author ? getAuthorMetadata(author, parsePage(query.page)) : {};
}

export default async function LegacyAuthorPage({ params, searchParams }: PageProps<"/author/[slug]">) {
  const [{ slug }, query] = await Promise.all([params, searchParams]);
  const author = await getAuthorBySlug(slug);
  if (!author) notFound();

  const page = parsePage(query.page);
  if (!isAuthorHref(author.href, slug)) permanentRedirect(getPageHref(author.href, page));
